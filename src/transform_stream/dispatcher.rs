    ...
    O: OutputSink,
{
        ...
        Lexeme<'i, T>: ToToken,
    {
        match lexeme.to_token(&mut self.delegate.capture_flags, self.encoding.get()) {
            ToTokenResult::Token(token) => {
                ...
                if let Err(e) = self.delegate.token_produced(token) {
                    // A suspended token is parked and serialized by `resume_dispatch`, so
                    // the lexeme is committed; any other error leaves it unconsumed for a
                    // graceful bail-out to flush raw.
            ...
            ToTokenResult::Text(text_type) => {
                ...
                let res = self.text_decoder.feed_text(
                    lexeme.spanned(),
                    false,
                ...
                if let Err(e) = res {
                    // See above: the decoder keeps the rest of a suspended text lexeme.
                    if matches!(e, RewritingError::Suspended) {
                        self.delegate.consume_lexeme(lexeme);
                    }
                    return Err(e.into());
    ...
    pub(crate) fn resume_dispatch(&mut self) -> Result<Option<ParserDirective>, RewritingError> {
        ...
        // The resumed handler for <meta charset> may have changed encoding.
        self.flush_encoding_change();
