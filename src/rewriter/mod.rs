...
use crate::rewritable_units::{
    Comment, Doctype, DocumentEnd, Element, EndTag, IncompleteUtf8Resync, TextChunk,
};
...
mod tests {
    ...
    mod suspension {
        ...
        #[test]
        fn document_end_handlers_queued_behind_a_suspension_still_run() {
            // Document-end handlers run last-registered first. The ones that
            // have not run yet when a handler suspends must survive until
            // the resume.
            let (out, n) = drive(
                &["<div></div>"],
                Settings {
                    document_content_handlers: vec![
                            ...
                            end.append("<first>", ContentType::Html);
                            Ok(())
                        }),
                        end!(|end| {
                            end.append("<second>", ContentType::Html);
                            SUSPEND()
                        }),
                    ],
                    ..Settings::new()
            ...
            assert_eq!(out, "<div></div><second><first>");
            assert_eq!(n, 1);
        }
